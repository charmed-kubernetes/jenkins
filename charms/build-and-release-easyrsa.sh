...
if [ ${RUN_TESTS} = true ]; then
  ...
  MODEL="build-and-release-easyrsa-${BUILD_NUMBER}"
  ...
  juju switch ${MODEL}
  bundletester -vF -l DEBUG -t ${JUJU_REPOSITORY}/builds/easyrsa -o report.xml -r xml
fi
...
if [ ${RELEASE} = true ]; then
  CHARM=$(/usr/bin/charm push charms/builds/easyrsa cs:~containers/easyrsa | head -n 1 | awk '{print $2}')
