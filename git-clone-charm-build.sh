...
if [ -n "${KUBE_VERSION}" ]; then
  MASTER="cd workspace/kubernetes/cluster/juju/layers/kubernetes-master && export KUBE_VERSION=${KUBE_VERSION} && ${CHARM_BUILD_CMD}"
else
  MASTER="cd workspace/kubernetes/cluster/juju/layers/kubernetes-master && ${CHARM_BUILD_CMD}"
fi
in-charmbox "${MASTER}"
