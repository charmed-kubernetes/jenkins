...
async def juju_wait(timeout_secs=None):
    cmd = f"juju wait -e {os.environ['JUJU_CONTROLLER']}:{os.environ['JUJU_MODEL']} -w"
    if timeout_secs:
...
async def set_config_and_wait(app, config, timeout_secs=None):
    ...
    async with assert_hook_occurs_on_all_units(app, "config-changed"):
        ...
        await juju_wait(timeout_secs=timeout_secs)
