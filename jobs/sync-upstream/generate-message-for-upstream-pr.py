...
def get_commit_list_to_merge(to_branch="staging", from_branch="master"):
    to_run = ["git", "log", "--format=%H %ct", "{}..{}".format(to_branch, from_branch)]
    commit_spew = check_output(to_run).decode("utf-8")
    commits = [
        (l.split()[0], datetime.fromtimestamp(float(l.split()[1])))
...
def write_pr_commit_message():
    ...
    print(
        "oldest timestamp is {} on commit {}".format(
            oldest_commit_timestamp[1].isoformat(), oldest_commit_timestamp[0]
        )
    )
    pr_list = [pr for pr in get_pr_list() if oldest_commit_timestamp < pr["created_at"]]
    ...
    for p in pr_list:
        ...
        if rn and oldest_commit_timestamp[1] < created:
            print(
                "found PR {} with date {} and release note '{}'".format(
            ...
            if rn:
                print(
                    "skipping PR {} with time {} due to date".format(
                        p["number"],
                        datetime.strptime(
                            p["created_at"], "%Y-%m-%dT%H:%M:%SZ"
                        ).isoformat(),
                    )
                )
            else:
                print(
                    "skipping PR {} with time {} due to lack of release note".format(
                        p["number"],
                        datetime.strptime(
                            p["created_at"], "%Y-%m-%dT%H:%M:%SZ"
                        ).isoformat(),
