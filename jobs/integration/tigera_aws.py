...
def juju(cmd, *args, json=True):
    ...
    cmd = ['juju', cmd, '-m', model] + list(args)
    return sh(cmd)
...
def bootstrap():
    ...
    # Create gateway
    gateway_id = ec2("create-internet-gateway")["InternetGateway"][
        "InternetGatewayId"
    ]
...
def get_machine_id(unit_name):
    ...
    while True:
        status = juju_json('status')
        machine_id = status['applications'][application]['units'][unit_name].get('machine')
        if machine_id:
            return machine_id
        time.sleep(1)
...
def deploy_bgp_router():
    ...
    for i in range(1, len(subnets)):
        ...
        result = ec2(
            ...
            '--subnet-id', subnet_id,
            '--groups', json.dumps(list(security_groups))
        )
        ...
        ec2(
            'modify-network-interface-attribute',
            '--network-interface-id', network_interface_id,
            '--attachment', 'AttachmentId=%s,DeleteOnTermination=true' % attachment_id
        )
    ...
    for i in range(1, len(subnets)):
        ...
        juju('ssh', 'router/0', 'sudo', 'dhclient', interface, '-e', 'IF_METRIC=101', json=False)
    ...
    juju(
        'model-config',
        'cloudinit-userdata="{postruncmd: [\\"%s\\"]}"' % cmd
    )
...
def configure_bgp():
    ...
    for unit_name in ['kubernetes-master/0', 'kubernetes-master/1']:
        ...
        while True:
            ...
            subordinate_names = [
                name for name in unit.get('subordinates', [])
    ...
    for subnet in subnets:
        ...
        ip = ips[0]
        subnet_bgp_peers[cidr] = [{
            'address': ip,
            'as-number': 64512
        }]

    unit_numbers = [
        int(unit_name.split('/')[1]) for unit_name in master_calico_units
    ]
    ...
    global_bgp_peers = [
        {'address': address, 'as-number': 64512}
        for address in master_ips
    ]
