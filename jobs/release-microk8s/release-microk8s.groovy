                    ...
                    channels.each { channel -> 
                        stage("Channel ${channel}") {
                            script {
                                ...
                                # Do some wating here to make sure the VM is ready.
                                while ! juju show-status -m ${juju_full_model} --format short ubuntu/0 | grep active ; do sleep 10 ; echo waiting... ; done
                                while ! juju ssh -m "${juju_full_model}" --pty=true ubuntu/0 -- "sudo echo EFS_ID=\$EFS_ID | sudo tee -a /etc/environment" ; do sleep 10 ; echo waiting...; done
